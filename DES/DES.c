...
void ByteToBit(bool * Data_out, char * Data_in, int number);                       //字节转换成位；
void Change_bit(bool * Data_out, int number);                                      //二进制的位置进行转换；
void BitToByte(char message[8], bool * Message_in, int number);                    //位转换成字节；
...
int main(void) {
    ...
    char result[8]  = {0};                      // 解密结果
    double Enc, Dec;
    clock_t start_time, Enc_time, Dec_time, End_time;
    ...
    Enc_time = clock();
    Enc = (double)((Enc_time - start_time) / CLOCKS_PER_SEC);
    printf("经过加密的密文为:\n");
    for (i = 0; i < 16; i++) {
        printf("%c ", cipher[i]);
    }
    ...
    Dec_time = clock();
    SetKey(Dec_Key);                            //生成16轮的解密子密钥；
    Run_DES(result, cipher, 1);                 //解密;
    End_time = clock();
    Dec = (double)((End_time - Dec_time) / CLOCKS_PER_SEC);
    printf("解密结果为:\n");
    for (i = 0; i < 8; i++) {
        printf("%c", result[i]);
    }
...
void Change_bit(bool * Data_out, int number) {                          // 二进制的位置进行转换；
	...
	for (i = 0; i < temp; i++) {
		Bitcopy(Temp, Data_out, temp);
		for (j = 0; j < temp; j++) {
			Data_out[j] = Temp[temp - 1 - j];
		}
		Data_out += temp;
	}
...
void ByteToBit(bool * Data_out, char * Data_in, int number) {          // 字节转位
	int i, j; 
	for (i = 0; i < number; i++) {
		Data_out[i] = (Data_in[i >> 3] >> (i & 7)) & 0x01;
	}
...
void Run_DES(char message[8], char HexMessage[16], int flag) {        //DES轮加密算法；
	int i; 
    ...
    if (flag == 0) {
        ByteToBit(Message_bit, message, 64);
        ...
        TableReplace(Message_bit, Message_bit, IPR_Table, 64);
	    BitToByte(message, Message_bit, 64);
    }
